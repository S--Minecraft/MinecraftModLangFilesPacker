...
exports.makeReadmeText = (json, type, title, escaped) ->
  ...
  if escaped? and escaped.length isnt 0
    ...
    for mod in escaped
      ...
      if mod.advanced? then readmeEscaped += " .minecraft/config/#{mod.advanced.path} - #{mod.version}"
      readmeEscaped += "\r\n"
    readmeEscaped += "\r\n"
  ...
  if type is 1 or type is 2
    for mod in json.mods
      if mod.advanced?
        ...
        if mod.advanced.type is "config" and type is 1 then readmePath += ".minecraftフォルダ/config/"
        if mod.advanced.type is "zip" then readmePath += "modのzipやjar/"
        readmePath += "#{advancedPath}\r\n"
  else if type is 0
    for mod in json.mods
      readmePath += "#{mod.name}(#{mod.version}): "
      if mod.path? then readmePath += util.ifObjToStr(mod.path)
      if mod.advanced?
        if mod.advanced.type is "config" then readmePath += " .minecraftフォルダ/config/"
        if mod.advanced.type is "zip" then readmePath += " modのzipやjar/"
        readmePath += util.ifObjToStr(mod.advanced.path)
      readmePath += "\r\n"
